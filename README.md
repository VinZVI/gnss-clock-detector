...
| POST | `/api/admin/etl` | Ручной запуск ETL (тело: `{"source": "ftp|nasa"}`) |

---

## Возможности аналитики (v0.5.0+)

### 📡 Мульти-ГНСС поддержка
Система автоматически собирает метаданные и статусы для всех глобальных созвездий:
- **ГЛОНАСС** (.glo), **GPS** (.gps), **BeiDou** (.bds), **Galileo** (.gal), **QZSS** (.qzs).
- Извлекаются заводские номера (НКУ), даты запуска, оценки и ввода в эксплуатацию.

### 🔄 Орбитальная корреляция
Для каждого спутника отслеживается его положение на орбите (Средняя аномалия, Высота).
- **Лепестковая диаграмма:** визуализация распределения аномалий по углу 0-360 градусов орбиты.
- **Статистика:** автоматический расчет корреляции Пирсона между высотой полета и шумом атомных часов.
- Позволяет выявлять физические причины сбоев (радиационные пояса, тень Земли).

### 📈 Стабильность частоты (ADEV)
- **Вариация Аллана:** расчет Overlapping Allan Deviation (ADEV) для оценки стабильности на разных интервалах усреднения.
- **Log-Log график:** профессиональная визуализация метрологических характеристик бортовых стандартов частоты.
...
### v0.5.0 — 2026-05-01 (Multi-GNSS & Physical Context)

**🪐 Орбитальная аналитика**
- ✅ **Орбитальная корреляция:** привязка аномалий часов к Кеплеровским элементам орбит (.oe).
- ✅ **Визуализация Polar Area:** круговые диаграммы распределения сбоев по орбитальному циклу.
- ✅ **Статистика:** расчет коэффициента корреляции Пирсона между высотой и шумом.

**📡 Глобальное покрытие**
- ✅ **Мульти-ГНСС:** полная поддержка BeiDou, Galileo и QZSS (паспорта, статусы здоровья).
- ✅ **Исправление дат:** переход на российский формат DD.MM.YY для файлов ГЛОНАСС-ИАЦ.
- ✅ **Стабильность (ADEV):** интеграция графиков вариации Аллана в UI.

**🛠️ Обслуживание**
- ✅ **Скрипт сброса:** `scripts/reset_metadata.py` для полной пересинхронизации паспортных данных.
- ✅ **Унификация UI:** исправлено отображение временных меток во всех таблицах.

---

### v0.4.0 — 2026-04-15 (Analytics & Core Pipeline Polish)

**📊 Аналитика и UI**
- ✅ **Аналитика стабильности:** вычисление RMS шума, дрейфа (НС/с), макс. скачков (чистый `numpy.diff`) и индекса здоровья (аппаратная надежность спутника) прямо в UI.
- ✅ **Динамический UI выбора источника:** добавлен селектор источника (FTP или NASA) в интерфейс запуска ETL.

**🚀 Backend & ETL Pipeline**
- ✅ **Улучшенный ETL API:** метод `POST /api/admin/etl` теперь принимает аргумент `"source"` (ftp/nasa) для выбора клиента загрузки вручную из веб-клиента. Планировщик (Q3) отключён за ненадобностью.
- ✅ **Модульность (устранение D1/B7):** написан обобщенный `utils.py` с единым методом `decompress()` для файлов .Z и .gz, решивший ошибки библиотек.
- ✅ **Идемпотентность БД:** безопасное обновление столбца `product_type` и `ON CONFLICT DO NOTHING` при вставке в `sat_clock` для защиты от дубликатов при переключении источников.
- ✅ **Поддержка rapid/final:** автоматическое распознавание `final`, `rapid` и `ultra` и корректный парсинг дней в названиях GPS-недель.
